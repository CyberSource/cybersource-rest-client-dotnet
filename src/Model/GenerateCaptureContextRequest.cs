...
namespace CyberSource.Model
{
    ...
    public partial class GenerateCaptureContextRequest :  IEquatable<GenerateCaptureContextRequest>, IValidatableObject
    {
        ...
        /// </summary>
        /// <value>The [target origin](https://developer.mozilla.org/en-US/docs/Glossary/Origin) of the website on which you will be launching Microform is defined by the scheme (protocol), hostname (domain) and port number (if used).    You must use https://hostname (unless you use http://localhost) Wildcards are NOT supported.  Ensure that subdomains are included. Any valid top-level domain is supported (e.g. .com, .co.uk, .gov.br etc)  Examples:   - https://example.com   - https://subdomain.example.com   - https://example.com:8080&lt;br&gt;&lt;br&gt;  If you are embedding within multiple nested iframes you need to specify the origins of all the browser contexts used, for example:    targetOrigins: [     \&quot;https://example.com\&quot;,     \&quot;https://basket.example.com\&quot;,     \&quot;https://ecom.example.com\&quot;   ]&lt;br&gt;&lt;br&gt;  You can supply up to nine origins within the targetOrigins field for nested iframes. If the list of origins exceeds five ensure that you:   - Compare the list of origins in the v2/sessions targetOrigins field against the location.ancestorOrigins of the browser.    - Ensure that the count of origins and their content matches in both.  If any origins are absent or mismatched, the system will prevent Microform from loading and display a client-side error message. </value>
        ...
        public override string ToString()
        {
            ...
            if (ClientVersion != null) sb.Append("  ClientVersion: ").Append(CyberSource.Utilities.SensitiveFieldMaskingUtility.MaskFieldValue("GenerateCaptureContextRequest", "clientVersion", ClientVersion.ToString())).Append("\n");
            if (TargetOrigins != null) sb.Append("  TargetOrigins: ").Append(CyberSource.Utilities.SensitiveFieldMaskingUtility.MaskFieldValue("GenerateCaptureContextRequest", "targetOrigins", TargetOrigins.ToString())).Append("\n");
