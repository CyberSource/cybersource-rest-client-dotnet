...
namespace CybsQaScript.TMS.CoreServices
{
    public class RetrieveAllPaymentInstruments
    {
        ...
        {
            // Initialize Api Name
            var apiFunctionName = "RetrieveAllPaymentInstruments";
            ...
            using (var csv =
                new CsvReader(new StreamReader(@"../../CSV_Files/TMS/CoreServices/RetrieveAllPaymentInstruments.csv"), true))
            {
                ...
                using (var writer = new CsvFileWriter(@"../../CSV_Files/TestReport/TestResults.csv"))
                {
                    ...
                    while (csv.ReadNextRecord())
                    {
                        ...
                        try
                        {
                            ...
                            var response = apiInstance.TmsV1InstrumentidentifiersTokenIdPaymentinstrumentsGet(profileId, tokenId);

                            if (response != null)
                            {
                                if (!response.Links.First.Href.Contains(tokenId))
                                {
                                    resultStatus = $"Assertion Failed: {clientConfig.ApiClient.ApiResponse.StatusCode}";
                                    resultMessage = "Token ID not contained in the URL";
                                }
                                else
                                {
