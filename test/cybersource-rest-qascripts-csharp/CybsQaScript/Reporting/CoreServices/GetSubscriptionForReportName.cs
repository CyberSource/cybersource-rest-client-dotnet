...
namespace CybsQaScript.Reporting.CoreServices
{
    ...
    {
        public static void GetSubsRepExec()
        {
            // Initialize Api Name
            var apiFunctionName = "GetSubscriptionForReportName";
            ...
            using (var csv =
                new CsvReader(new StreamReader(@"../../CSV_Files/Reporting/CoreServices/GetSubscriptionForReportName.csv"), true))
            {
                ...
                using (var writer = new CsvFileWriter(@"../../CSV_Files/TestReport/TestResults.csv"))
                {
                    ...
                    while (csv.ReadNextRecord())
                    {
                        ...
                        try
                        {
                            ...
                            var response = apiInstance.GetSubscription(reportName);

                            if (response != null)
                            {
                                if (reportName != response.ReportName)
                                {
                                    resultStatus = $"Assertion Failed: {clientConfig.ApiClient.ApiResponse.StatusCode}";
                                    resultMessage = "Fails due to mismatch in ReportName";
                                }
                                else
                                {
                                    resultStatus = $"Pass:{clientConfig.ApiClient.ApiResponse.StatusCode}- {response.ReportName}";
