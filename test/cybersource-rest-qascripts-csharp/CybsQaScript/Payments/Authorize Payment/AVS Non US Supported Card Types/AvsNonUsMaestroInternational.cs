...
using System;
//AvsNonUsMaestroInternational

namespace CybsQaScript.Payments.Authorize_Payment.AVS_Non_US_Supported_Card_Types
{
    public class AvsNonUsMaestroInternational
    {
        public static void CreatePaymentExec()
        {
            // Initialize Api Name
            var apiFunctionName = "AvsNonUsMaestroInternational";
            ...
            using (var csv =
                new CsvReader(new StreamReader(@"../../CSV_Files/Payment/AuthorizePayment/AVSNonUsSupportedCardTypes/CreateMaestroInternational.csv"), true))
            {
                ...
                using (var writer = new CsvFileWriter(@"../../CSV_Files/TestReport/TestResults.csv"))
                {
                    ...
                    while (csv.ReadNextRecord())
                    {
                        ...
                        try
                        {
                            ...
                            var consumerAuthenticationInformationObj = new Ptsv2paymentsConsumerAuthenticationInformation
                            {
